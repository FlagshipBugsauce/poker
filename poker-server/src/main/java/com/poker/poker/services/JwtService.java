...
public class JwtService {
  ...
  private Claims extractAllClaims(String token) {
    return Jwts.parser()
        .setSigningKey(appConstants.getJwtSecretKey())
  ...
  private String createToken(Map<String, Object> claims, String subject) {
    return Jwts.builder()
        ...
        .setExpiration(
            new Date(System.currentTimeMillis() + appConstants.getTokenDurationInMillis()))
  ...
  /**
   * Cleans the "Bearer " prefix if it is there, so that if a token is provided which has the
   * "Bearer " prefix, it wont cause any problems.
   *
